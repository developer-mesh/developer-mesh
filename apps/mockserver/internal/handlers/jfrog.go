...
func handleRepositories(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		json.NewEncoder(w).Encode(map[string]string{"status": "Repository created successfully"})
	}
}
...
func handleProjects(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		w.WriteHeader(http.StatusCreated)
		json.NewEncoder(w).Encode(map[string]string{"status": "Project created successfully"})
	}
}
